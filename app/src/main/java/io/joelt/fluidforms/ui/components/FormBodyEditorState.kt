...
// Cursors for indexing form body
...
// Other extensions
private fun TextRange.toCursorRange(formBody: List<Either<String, Slot>>): CursorRange =
    CursorRange(formBody.cursorAt(this.start), formBody.cursorAt(this.end - 1))

private fun String.insert(position: Int, value: String) =
    this.substring(0, position) + value + this.substring(position)

data class FormBodyEditorState(
    val formBody: List<Either<String, Slot>>,
    ...
    val annotatedString: AnnotatedString
        ...
        get() {
            ...
            return formBody.annotateSlotsIndexed { index, slot ->
    ...
    private fun delete(selection: TextRange): List<Either<String, Slot>> {
        ...
        val newBody = formBody.toMutableList()
        val (start, end) = selection.toCursorRange(formBody)
        if (start.itemIndex == end.itemIndex) {
            val item = formBody[start.itemIndex]
            val newItem = item.withNewValue {
        ...
        val newStartItem = formBody[start.itemIndex].withNewValue {
            it.substring(0, start.subIndex)
        }
        val newEndItem = formBody[end.itemIndex].withNewValue {
    ...
    private fun replace(selection: TextRange, value: String): List<Either<String, Slot>> {
        ...
        val (start, end) = selection.toCursorRange(formBody)
        if (formBody[start.itemIndex] is Either.Right
            && start.itemIndex == end.itemIndex
            && start.subIndex == 0
            && end.subIndex == formBody[start.itemIndex].text.lastIndex
        ) {
            // If the deleted portion is a whole slot, restore it
            val newSlot =
                (formBody[start.itemIndex] as Either.Right).value.makeCopy(displayLabel = value)
    ...
    private fun selectAdjacentSlot(): FormBodyEditorState? {
        if (!isCursor) {
            return null
        }

        val position = selection.start
        var cursor = formBody.cursorAt(position)
        if (selectedSlotIndex == null) {
            if (position == text.length) {
            ...
            val precedingItem = formBody.getOrNull(cursor.itemIndex - 1)
            if (precedingItem is Either.Right) {
                return FormBodyEditorState(
                    formBody,
                    selection,
                    composition,
                    cursor.itemIndex - 1
        ...
        if (cursor.itemIndex == selectedSlotIndex && cursor.subIndex == 0) {
            return FormBodyEditorState(formBody, selection, composition, null)
        }
        return null
    }

    private fun unselectCurrentSlotWhenEnterPressed(newText: String): FormBodyEditorState? {
        if (selectedSlotIndex == null || newText != "\n") {
            return null
        }

        val cursor = formBody.cursorAt(selection.start)
        val endOfSlot = if (selectedSlotIndex != cursor.itemIndex) {
            ...
            selection.start + formBody[cursor.itemIndex].text.length - cursor.subIndex
        }

        return FormBodyEditorState(formBody, TextRange(endOfSlot), null, null)
    }

    // Change state functions
    fun withNewTextFieldValue(newTextFieldValue: TextFieldValue): FormBodyEditorState {
        val textChanged = newTextFieldValue.text != text
        val lenDiff = newTextFieldValue.text.length - textFieldValue.text.length
        val newBody = if (formBody.isEmpty()) {
            ...
            insert(formBody, cursor, newText)
    ...
    fun insertSlotAtSelection(slot: Slot): FormBodyEditorState {
